...
app.post("/api/telenor/activate", async (req, res) => {
  try {
    ...
    if (!msisdn || !otp) {
      return res.status(400).json({
        success: false,
        message: "msisdn and otp required"
      });
    }

    const form = new URLSearchParams();
    form.append("action", "wptp_mydjuice_products_activation");
    form.append("security", process.env.TELENOR_NONCE);
    ...
    const response = await fetch(
      "https://www.telenor.com.pk/wp-admin/admin-ajax.php",
      {
        ...
        headers: {
          ...
          "cookie": process.env.TELENOR_COOKIE
        },
        body: form
      }
    );
